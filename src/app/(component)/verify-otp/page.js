  ...
  const verifyOTP = async () => {
    ...
    if (enteredOTP === storedOTP) {
      ...
      try {
        ...
        // Clear the OTP from session storage
        sessionStorage.removeItem('verificationOTP')

        // Set completion flag in session storage
      ...
      <div className="bg-white p-6 md:p-8 lg:p-12 md:w-2/3 flex items-center justify-center">
        <div className="w-full max-w-md space-y-8">
          ...
          <div className="space-y-8">
            ...
            <Button
              className="w-full bg-[#0F172A] text-white hover:bg-[#1E293B] transition-all duration-200"
              disabled={loading}
              onClick={verifyOTP}
            >
